...
- 1.3.1 좌측 중간에 위치한 `[보안 그룹]`을 클릭 후 `[보안 그룹 생성]`을 클릭하고 다음 정보를 입력 (총 4개의 보안 그룹 생성)
    - `[보안 그룹 이름]` : **ext ELB**
    - `[설명]` : **ext ELB**
    - `[VPC]` : **testVPC**
- 1.3.2 인바운드 규칙 추가
...
- 1.3.4 `[소스]`에서 스크롤을 내려 `위치 무관`을 클릭
    ...
    - `[VPC]` : **testVPC**
- 1.3.5 인바운드 규칙 추가
- 1.3.6 `[유형]`에서 스크롤을 내려 `SSH`를 클릭
- 1.3.7 `[소스]`에서 스크롤을 내려 `내 IP`를 클릭
    - `[보안 그룹 이름]` : **WEB**
    - `[설명]` :  **WEBt**
    - `[VPC]` : **testVPC**
- 1.3.8 인바운드 규칙 추가
- 1.3.9 `[유형]`에서 스크롤을 내려 `HTTP`를 클릭
- 1.3.10 `[소스]`에서 스크롤을 내려 `위치 무관`을 클릭
- 1.3.11 `[유형]`에서 스크롤을 내려 `SSH`를 클릭
- 1.3.12 `[소스]`옆 검색에서 보안 그룹 중 **Bastion Host** 를 선택
    - `[보안 그룹 이름]` : **RDS**
    - `[VPC]` : **testVPC**
- 1.3.13 인바운드 규칙 추가
- 1.3.14 `[유형]`에서 스크롤을 내려 `MYSQL/Aurora`를 클릭
- 1.3.15 `[소스]`옆 검색에서 보안 그룹 중 **Bastion Host** 를 선택
- 1.3.16 `[유형]`에서 스크롤을 내려 `MYSQL/Aurora`를 클릭
- 1.3.17 `[소스]`옆 검색에서 보안 그룹 중 **WEB** 을 선택
